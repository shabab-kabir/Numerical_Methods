function bezier_draw_3d
    ...
    function mouseClick(~, ~)
        ...
        z = 5;  % Fixed Z-coordinate for simplicity
    ...
    function addOrMovePoint(x, y, z)
        ...
        for i = 1:size(points{currentCurve},1)
            dist = sqrt((x - points{currentCurve}(i,1))^2 + (y - points{currentCurve}(i,2))^2 + (z - points{currentCurve}(i,3))^2);
            if dist < minDist
                minDist = dist;
                minIndex = i;
            end
        ...
        if minDist < 0.5
            points{currentCurve}(minIndex,:) = [x, y, z];
        else
            % Add new point
            points{currentCurve} = [points{currentCurve}; x, y, z];
        end
    ...
    function updatePlots()
        ...
        for c = 1:currentCurve
            if ~isempty(points{c})
                % Plot control points
                plot3(ax, points{c}(:,1), points{c}(:,2), points{c}(:,3), 'ro-');
                % Plot Bézier curve
    ...
    function plotBezierCurve(ctrlPoints)
        t = linspace(0, 1, 100);
        bezierPoints = zeros(length(t), 3);
        
        for i = 1:length(t)
            bezierPoints(i,:) = computeBezierPoint(ctrlPoints, t(i));
        end
        
        plot3(ax, bezierPoints(:,1), bezierPoints(:,2), bezierPoints(:,3), 'b-');
    end
